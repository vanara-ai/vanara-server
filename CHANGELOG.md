...
- Test suite covering PDF Unicode sanitization, pydantic schema validators, `get_groq_key` header enforcement, and `invoke_with_retry` transient-failure handling
...
- PyMuPDF fallback for PDFs with broken font encoding (auto-detects when PyPDF2 output has <3% spaces, falls back seamlessly)
- Brand-aligned user-facing error messages via error classifier (internal errors mapped to helpful copy)

### Fixed
- PDF text extraction failures on resumes with certain font encodings (PyPDF2 produces no-space text)
